    ...
    .setAction(async (taskArguments, hre) => {
        try {
            ...
            const answer = await readline.question(
                `Are you sure you wish to set ${taskArguments.newAdmin} as the new admin? (Y/y)`
            );
            ...
            const currentAdmin = await hre.ethers.getSigner(
                taskArguments.currentAdmin
            );

            await transferAccountOwnership(
                currentAdmin,
                taskArguments.newAdmin,
                hre
            );
...
async function transferAccountOwnership(
    ...
    try {
        ...
        logger.out(
            `Failed to change admin for Account diamond, reason: ${error}`,
            logger.Level.Error
        );
    }
}
...
async function changeProxiesAdmin(
    ...
    for (const proxy of proxies) {
        try {
            const upgradeableProxy =
                ITransparentUpgradeableProxy__factory.connect(
                    proxy.address,
                    currentAdmin
                );
            const tx = await upgradeableProxy.changeAdmin(
                taskArguments.newAdmin
            );
            await hre.ethers.provider.waitForTransaction(tx.hash);
            ...
            logger.out(
                `Failed to change admin for ${proxy.name}, reason: ${error}`,
                logger.Level.Error
            );
        }
    }
...
function extractProxyContractAddresses(
    ...
    return Object.entries(value).flatMap(([key, val]) =>
        extractProxyContractAddresses(key, val)
    );
}
