...
export async function deployFundraising(FundraisingDataInput: FundraisingMessage.InstantiateMsgStruct, ANGEL_CORE_STRUCT: string, verify_contracts: boolean, hre: HardhatRuntimeEnvironment) {
    try {
      ...
      const FundraisingData = FundraisingInstance.interface.encodeFunctionData('initFundraising',[FundraisingDataInput]);
  
      const FundraisingProxy = await ProxyContract.deploy(FundraisingInstance.address, proxyAdmin.address, FundraisingData);
  
      await FundraisingProxy.deployed();
 
      
      let FundraisingAddress = {
        FundraisingProxy: FundraisingProxy.address,
        FundraisingImplementation: FundraisingInstance.address,
        FundraisingLibInstance: FundraisingLibInstance.address
      }
      await saveFrontendFiles({FundraisingAddress});
