...
//    */
//   function initFundraising(FundraisingMessage.InstantiateMsg memory details) public {
...
//    */
//   function getRegistrarConfig() public view returns (RegistrarStorage.Config memory) {
...
//   ) public payable {
//     if (balance.coinNativeAmount == 0 && balance.cw20Addr == address(0)) revert("Invalid inputs");
//     // receive tokens
...
//     if (registrarConfig.accountsContract == address(0)) revert("accountsContract does not exist");
//     if (registrarConfig.accountsContract != accountsContract) revert("Unauthorized");

//     AccountStorage.Endowment memory endowment_details = IAccountsQuery(accountsContract)
//       .queryEndowmentDetails(endowmentId);
//     if (endowment_details.status != AngelCoreStruct.EndowmentStatus.Approved)
//       revert("Unauthorized");

//     // assert that the campaign has started already
//     if (message.endTimeEpoch - config.campaignPeriodSeconds <= block.timestamp) {
...
//     require(balance.coinNativeAmount == msg.value, "Native coin amount mismatch");

//     AngelCoreStruct.GenericBalance memory lockedBalance = AngelCoreStruct.GenericBalance({
...
//         (message.fundingGoal.coinNativeAmount * message.rewardThreshold) /
//         100;
...
//     ) {
//       int256 pos = -1;
//       for (uint256 i = 0; i < config.acceptedTokens.Cw20CoinVerified_addr.length; i++) {
//         if (
...
//         (message.fundingGoal.Cw20CoinVerified_amount[0] * message.rewardThreshold) /
//         100;
//       fundingThreshold.Cw20CoinVerified_addr[0] = message.fundingGoal.Cw20CoinVerified_addr[0];
...
//       contributedBalance.Cw20CoinVerified_addr[0] = message.fundingGoal.Cw20CoinVerified_addr[0];
//     } else {
//       revert("Contract only accepts a single CW20 or Native Token at a time for funding goal");
//     }
...
//     if (state.CAMPAIGNS[id].creator == address(0)) revert("Campaign does not exist");

//     if (!FundraisingLib.isExpired(state.CAMPAIGNS[id])) revert("Not expired");
//     RegistrarStorage.Config memory registrarConfig = getRegistrarConfig();

//     Campaign storage campaign = state.CAMPAIGNS[id];

//     if (FundraisingLib.thresholdMet(campaign.contributedBalance, campaign.fundingGoal)) {
...
//       ) = FundraisingLib.calculateWitholding(state.config.taxRate, campaign.contributedBalance);
...
//     if (!campaign.open || FundraisingLib.isExpired(campaign)) revert("Campaign is expired");
//     if (msg.sender != campaign.creator) revert("Unauthorized");

//     // ensure the token is on the allowlist
//     bool found = false;
//     for (uint256 i = 0; i < campaign.fundingGoal.Cw20CoinVerified_addr.length; i++) {
//       if (campaign.fundingGoal.Cw20CoinVerified_addr[i] == balance.cw20Addr) {
//         found = true;
//         break;
//       }
//     }
...
//     AngelCoreStruct.addToken(campaign.lockedBalance, balance.cw20Addr, balance.cw20Amount);
//     emit FundraisingCampaignUpdated(id, campaign);
//   }
...
//    * @param balance Balance
//    */
//   function executeContribute(uint256 id, Balance memory balance) public payable {
//     if (balance.coinNativeAmount == 0 && balance.cw20Addr == address(0)) revert("Invalid inputs");
//     // receive tokens
//     bool success = IERC20(balance.cw20Addr).transferFrom(
//       msg.sender,
...
//     mapping(uint256 => FundraisingStorage.ContributorInfo) storage contributor = state.CONTRIBUTORS[
//       msg.sender
//     ];
//     if (state.CONTRIBUTOR_CAMPAIGNS[msg.sender].length != 0) {
//       if (contributor[id].exists) {
//         contributor[id].balance.coinNativeAmount += balance.coinNativeAmount;
//         AngelCoreStruct.addToken(contributor[id].balance, balance.cw20Addr, balance.cw20Amount);
//       } else {
...
//       AngelCoreStruct.GenericBalance memory default_bal;
//       default_bal.coinNativeAmount += balance.coinNativeAmount;
//       default_bal = AngelCoreStruct.addTokenMem(default_bal, balance.cw20Addr, balance.cw20Amount);
...
//     campaign.contributedBalance.coinNativeAmount += balance.coinNativeAmount;
//     AngelCoreStruct.addToken(campaign.contributedBalance, balance.cw20Addr, balance.cw20Amount);
...
//     if (state.CONTRIBUTORS[msg.sender][id].contributionRefunded) revert("Already refunded");
//     state.CONTRIBUTORS[msg.sender][id].contributionRefunded = true;
//     sendTokens(msg.sender, state.CONTRIBUTORS[msg.sender][id].balance);
//     emit FundraisingContributorUpdated(id, msg.sender, state.CONTRIBUTORS[msg.sender][id]);
...
//     uint256 allCoins = campaign.contributedBalance.Cw20CoinVerified_addr.length;
//     AngelCoreStruct.GenericBalance memory rewards = AngelCoreStruct.GenericBalance(
//       0,
//       new uint256[](allCoins),
//       new address[](allCoins)
//     );

//     if (campaign.contributedBalance.coinNativeAmount == 0) rewards.coinNativeAmount = 0;
//     else {
//       rewards.coinNativeAmount =
//         (contributor[id].balance.coinNativeAmount * campaign.lockedBalance.coinNativeAmount) /
//         campaign.contributedBalance.coinNativeAmount;
//     }
//     rewards.Cw20CoinVerified_addr = campaign.contributedBalance.Cw20CoinVerified_addr;

//     for (uint256 i = 0; i < allCoins; i++) {
//       uint256 lockedBalanceAmt = 0;
//       for (uint256 j = 0; j < campaign.lockedBalance.Cw20CoinVerified_addr.length; j++) {
//         if (campaign.lockedBalance.Cw20CoinVerified_addr[j] == rewards.Cw20CoinVerified_addr[i]) {
//           lockedBalanceAmt = campaign.lockedBalance.Cw20CoinVerified_amount[j];
//           break;
//         }
//       }
//       uint256 contributorAmt = 0;
//       for (uint256 j = 0; j < contributor[id].balance.Cw20CoinVerified_addr.length; j++) {
//         if (contributor[id].balance.Cw20CoinVerified_addr[j] == rewards.Cw20CoinVerified_addr[i]) {
//           contributorAmt = contributor[id].balance.Cw20CoinVerified_amount[j];
//           break;
//         }
//       }
...
//       rewards.Cw20CoinVerified_addr[i] = campaign.contributedBalance.Cw20CoinVerified_addr[i];
//     }
...
//    */
//   function sendTokens(address to, AngelCoreStruct.GenericBalance memory balance) internal {
...
//       require(
//         IERC20(balance.Cw20CoinVerified_addr[i]).transfer(to, balance.Cw20CoinVerified_amount[i]),
//         "Transfer failed"
//       );
//     }
//   }
...
//     FundraisingMessage.DetailsResponse memory details = FundraisingMessage.DetailsResponse({
//       id: id,
...
//   function queryConfig() public view returns (FundraisingStorage.Config memory) {
//     return state.config;
//   }
