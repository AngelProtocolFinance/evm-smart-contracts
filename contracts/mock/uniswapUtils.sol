...
contract UniswapUtils {
  ...
  function getTokenOrder(
    address tokenA,
    address tokenB
  ) internal pure returns (address token0, address token1, bool isPtLower) {
  ...
  function createPoolAndMintPositionErC20(
    createUniswapPoolERC20Args memory details
  ) public returns (address) {
    bool success;
    require(
      IERC20(details.projectToken).balanceOf(msg.sender) >= details.amountA,
    ...
    success = IERC20(details.projectToken).transferFrom(msg.sender, address(this), details.amountA);
    require(success, "transferFrom failed");
    success = IERC20(details.usdcToken).transferFrom(msg.sender, address(this), details.amountB);
    require(success, "transferFrom failed");
    ...
    address tokenB;
    bool isPtLower;
    (tokenA, tokenB, isPtLower) = getTokenOrder(details.projectToken, details.usdcToken);
    ...
    (amount0, amount1) = isPtLower ? (amount0, amount1) : (amount1, amount0);
    // Remove allowance and refund in both assets.
    if (amount0 < details.amountA) {
      success = IERC20(details.projectToken).approve(address(nonfungiblePositionManager), 0);
      require(success, "approve failed");
    ...
    if (amount1 < details.amountB) {
      success = IERC20(details.usdcToken).approve(address(nonfungiblePositionManager), 0);
      require(success, "approve failed");
  ...
  function createPoolAndMintPosition(
    createUniswapPoolArgs memory details
  ) public payable returns (address) {
    require(
      IERC20(details.tokenA).balanceOf(msg.sender) >= details.amountA,
      "Insufficient tokenA balance"
    );

    bool success = IERC20(details.tokenA).transferFrom(msg.sender, address(this), details.amountA);
    require(success, "transferFrom failed");

    success = IERC20(details.tokenA).approve(address(nonfungiblePositionManager), details.amountA);
    require(success, "approve failed");
    success = IERC20(details.tokenB).approve(address(nonfungiblePositionManager), msg.value);
    require(success, "approve failed");
    ...
    address tokenB;
    bool isPtLower;
    (tokenA, tokenB, isPtLower) = getTokenOrder(details.tokenA, details.tokenB);

    {
      uint256 tokenAAmount = isPtLower ? details.amountA : msg.value;
      uint256 tokenBAmount = isPtLower ? msg.value : details.amountA;
    ...
    // Remove allowance and refund in both assets.
    if (amount0 < details.amountA) {
      success = IERC20(details.tokenA).approve(address(nonfungiblePositionManager), 0);
      require(success, "approve failed");
      uint256 refund = details.amountA - amount0;
    ...
    if (amount1 < msg.value) {
      success = IERC20(details.tokenB).approve(address(nonfungiblePositionManager), 0);
      require(success, "approve failed");
