...
contract Staking is Initializable, ERC20, Pausable, ReentrancyGuard, Ownable {
    ...
    function unstake(
        ...
        require(
            stakeInfos[msg.sender][stakeId].endTs < block.timestamp,
            "Stake not ended"
        );
        require(
            amount <=
                stakeInfos[msg.sender][stakeId].amount -
                    stakeInfos[msg.sender][stakeId].claimed,
            "Invalid amount"
        );
        ...
        require(
            IERC20(haloToken).transfer(msg.sender, totalTokens),
            "Transfer failed"
        );
