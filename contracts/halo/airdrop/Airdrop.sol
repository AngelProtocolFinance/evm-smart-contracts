...
contract Airdrop is Storage, Initializable, ReentrancyGuard {
    event AirdropUpdateConfig(address owner);
    event AirdropRegisterMerkleRoot(uint256 stage, bytes32 merkleRoot);
    event AirdropClaim(uint256 stage, address sender, uint256 amount);
    ...
    function updateConfig(address owner) public nonReentrant {
        ...
        emit AirdropUpdateConfig(owner);
    }
    ...
    function registerMerkleRoot(bytes32 merkleRoot) public nonReentrant {
        require(
            state.config.owner == msg.sender,
            "only owner can register merkle root"
        );
        state.MerkleRoots[state.latestStage] = merkleRoot;
        state.latestStage += 1;
        emit AirdropRegisterMerkleRoot(state.latestStage - 1, merkleRoot);
    }
    ...
    function claim(
        ...
        require(
            !state.isClaimed[state.latestStage][msg.sender],
            "already claimed"
        );
        ...
        emit AirdropClaim(state.latestStage, msg.sender, amount);
    }
    ...
    function queryConfig()
        public
        view
        returns (AirdropMessage.ConfigResponse memory)
    {
        return
            AirdropMessage.ConfigResponse({
    ...
    function queryMerkleRoot(
        ...
        return
            AirdropMessage.MerkleRootResponse({
                stage: stage,
                merkleRoot: state.MerkleRoots[stage]
            });
    }
