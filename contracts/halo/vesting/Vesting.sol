...
contract Vesting is Ownable, ReentrancyGuard {
  ...
  function withdraw(uint256 vestingId) public nonReentrant {
    if (vesting[msg.sender][vestingId].claimed == vesting[msg.sender][vestingId].amount) {
      revert("Nothing to claim");
    }
    uint256 timediff = min(block.timestamp, vesting[msg.sender][vestingId].endTime) -
    ...
    uint256 claimable = (vesting[msg.sender][vestingId].amount * timediff * timediff) /
      (vestingDuration * vestingDuration);
    require(claimable > vesting[msg.sender][vestingId].claimed, "Nothing to claim");
    require(
      IERC20(haloToken).transfer(msg.sender, claimable - vesting[msg.sender][vestingId].claimed),
      "Transfer failed"
    );
