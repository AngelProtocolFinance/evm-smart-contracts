...
export async function Vesting(proxyAdmin = ADDRESS_ZERO, VestingDataInput: VestingMessage.InstantiateMsgStruct, hre: HardhatRuntimeEnvironment) {
    try {
      ...
      const VestingData = VestingInstance.interface.encodeFunctionData('initialize',[VestingDataInput]);
  
      const VestingProxy = await ProxyContract.deploy(VestingInstance.address, proxyAdmin, VestingData);
