...
export async function Community(proxyAdmin = ADDRESS_ZERO, CommunityDataInput: CommunityMessage.InstantiateMsgStruct, hre: HardhatRuntimeEnvironment) {
    try {
      ...
      const CommunityData = CommunityInstance.interface.encodeFunctionData('initialize',[CommunityDataInput]);
  
      const CommunityProxy = await ProxyContract.deploy(CommunityInstance.address, proxyAdmin, CommunityData);
