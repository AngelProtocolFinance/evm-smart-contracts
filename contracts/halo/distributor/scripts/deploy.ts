...
export async function distributor(
  ...
  DistributorDataInput: DistributorMessage.InstantiateMsgStruct,
  ...
  try {
    ...
    const DistributorData = DistributorInstance.interface.encodeFunctionData("initialize", [
      DistributorDataInput,
    ]);

    const DistributorProxy = await ProxyContract.deploy(
