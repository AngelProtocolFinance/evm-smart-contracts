...
export async function GovHodler(proxyAdmin = ADDRESS_ZERO, GovHodlerDataInput: GovHodlerMessage.InstantiateMsgStruct, hre: HardhatRuntimeEnvironment) {
    try {
      ...
      const GovHodlerData = GovHodlerInstance.interface.encodeFunctionData('initialiaze',[GovHodlerDataInput]);
  
      const GovHodlerProxy = await ProxyContract.deploy(GovHodlerInstance.address, proxyAdmin, GovHodlerData);
