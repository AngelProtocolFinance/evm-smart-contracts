...
contract DonationMatch is Storage, Initializable {
  ...
  function executeDonorMatch(
    ...
    require(
      IERC20(token).approve(state.config.reserveToken, reserveTokenAmount),
    ...
    if (token == state.config.reserveToken) {
      ...
      require(
        IERC20(token).approve(registrar_config.accountsContract, endowmentAmount),
        "Approve failed"
      );
      ...
      require(
        IERC20(state.config.reserveToken).approve(token, reserveTokenAmount),
        "Approve failed"
      );
