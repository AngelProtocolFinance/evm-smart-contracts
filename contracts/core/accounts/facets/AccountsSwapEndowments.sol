...
contract AccountsSwapEndowments is ReentrancyGuardFacet, AccountsEvents {
  ...
  function swapToken(
    ...
    require(IERC20(tokenIn).balanceOf(address(this)) >= amountIn, "BalanceTooSmall");
    ...
    require(IERC20(tokenIn).approve(registrar_config.swapsRouter, amountIn), "Approve failed");

    uint256 amountOut = ISwappingV3(registrar_config.swapsRouter).executeSwaps(
      tokenIn,
      amountIn,
      tokenOut,
  ...
  function updateStateBalance(
    ...
    if (accountType == AngelCoreStruct.AccountType.Locked) {
      AngelCoreStruct.addToken(state.STATES[id].balances.locked, tokenAddress, amount);
    } else {
      AngelCoreStruct.addToken(state.STATES[id].balances.liquid, tokenAddress, amount);
    }
  }
}
