...
interface IAccounts {
  ...
  function updateConfig(address newRegistrar, uint256 maxGeneralCategoryId) external returns (bool);
  ...
  ///QUERY FUNCTION
  function queryConfig() external view returns (AccountMessages.ConfigResponse memory);

  function queryState(uint32 id) external view returns (AccountMessages.StateResponse memory);

  function queryEndowmentDetails(uint32 id) external view returns (AccountStorage.Endowment memory);
}
