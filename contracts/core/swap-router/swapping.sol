...
contract SwapRouter is Storage {
   ...
   */
  function initSwapRouter(SwapRouterMessages.InstantiateMsg memory details) public {
   ...
   */
  function checkPoolAndReturFee(address tokena, address tokenb) internal view returns (uint24) {
    ...
    for (uint256 i = 0; i < 3; i++) {
      tempAddress = IPool(swapFactory).getPool(tokena, tokenb, swappingFees[i]);
  ...
  function swap(
    ...
    require(IERC20(tokena).transferFrom(msg.sender, address(this), amount), "TransferFrom failed");

    require(IERC20(tokena).approve(swapRouter, amount), "Approve failed");
  ...
  function executeSwapOperations(
    ...
    amountPossible += swap(tokenIn, tokenOut, amountIn, address(this));

    require(amountPossible >= amountOut, "Output funds less than the minimum funds");

    require(IERC20(tokenOut).transfer(msg.sender, amountPossible), "Transfer failed");
   ...
   */
  function swapTokenToUsdc(address tokena, uint256 amountin) public returns (uint256) {
    require(
      msg.sender == config.registrarContract || msg.sender == config.accountsContract,
      "Unauthorized"
    );
    ...
    uint256 account = swap(tokena, registrar_config.usdcAddress, amountin, msg.sender);
  ...
  function swapEthToToken() public payable returns (uint256) {
    ...
    //Get pool
    uint24 fees = checkPoolAndReturFee(registrar_config.wethAddress, registrar_config.usdcAddress);
    ...
    // The call to `exactInputSingle` executes the swap.
    uint256 amountOut = ISwapRouter(swapRouter).exactInputSingle{value: msg.value}(params);
