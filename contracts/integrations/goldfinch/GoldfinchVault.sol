...
contract GoldfinchVault is IVault, IERC721Receiver {
  ...
  modifier nonzeroPositionOnly(uint32 accountId) {
    ...
    IStakingRewards.StakedPosition memory position = stakingPool.getPosition(
      tokenIdByAccountId[accountId]
    );
  ...
  function _isSiblingVault() internal view returns (bool) {
    LocalRegistrarLib.StrategyParams memory stratParams = registrar.getStrategyParamsById(
      STRATEGY_ID
    );
    ...
    returns (IRouter.RedemptionResponse memory)
  {
    ...
    (uint256 redeemedUSDC, uint256 redeemedFIDU) = _redeemFiduForUsdc(
      accountId,
      position.amount,
      amt
  ...
  function redeemAll(
    uint32 accountId
  ) external payable override approvedRouterOnly nonzeroPositionOnly(accountId) returns (uint256) {
    IStakingRewards.StakedPosition memory position = stakingPool.getPosition(
      tokenIdByAccountId[accountId]
    );
    LocalRegistrarLib.AngelProtocolParams memory apParams = registrar.getAngelProtocolParams();
  ...
  function harvest(uint32[] calldata accountIds) external override approvedRouterOnly {
    AngelCoreStruct.FeeSetting memory feeSetting = registrar.getFeeSettingsByFeeType(
      AngelCoreStruct.FeeTypes.Harvest
    );
  ...
  function _harvestSingle(
    ...
    (redeemedUSDC, dFidu) = _redeemFiduForUsdc(accountId, position.amount, (tax + rebalAmt)); // Redeem FIDU from underlying to USDC
    principleByAccountId[accountId].fiduP -= dFidu; // Deduct fidu redemption from principle
  ...
  function _updatePrinciples(uint32 accountId, uint256 redeemedFIDU) internal {
    uint256 percentOfPRedeemed = (redeemedFIDU * PRECISION) /
      (principleByAccountId[accountId].fiduP);
    uint256 redemptionLessYield = (percentOfPRedeemed * principleByAccountId[accountId].usdcP) /
      PRECISION;
    // update principles
  ...
  function _calcSlippageTolernace(
    uint256 i,
    uint256 j,
    uint256 dx,
  ...
  function _calcYield_withPrecision(
    uint32 accountId,
  ...
  function _calcTax(
    ...
    return
      ((yield_withPrecision * taxableAmt * feeSetting.bps) / AngelCoreStruct.FEE_BASIS) / PRECISION;
  }

  function _redeemFiduForUsdc(
    uint32 accountId,
    uint256 positionAmount,
    uint256 desiredUsdc
  ...
  function _getExchageRate_withPrecision(
    uint256 i,
    uint256 j,
    uint256 amt
  ) internal view returns (uint256) {
