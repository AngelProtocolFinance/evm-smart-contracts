...
contract FluxStrategy is IStrategy, Pausable, ReentrancyGuard {
  ...
  function deposit(uint256 amt) external payable whenNotPaused nonReentrant nonZeroAmount(amt) returns (uint256) {
  ...
  function withdraw(uint256 amt) external payable whenNotPaused nonReentrant nonZeroAmount(amt) returns (uint256) {
