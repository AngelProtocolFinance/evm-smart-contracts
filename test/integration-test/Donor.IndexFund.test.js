...
describe('Donor donates from Index Fund', function () {
	...
	it('Should create an Index Fund and donate to it', async function () {
		...
		await MockERC20.connect(deployer).transfer(addrs[0].address, ethers.utils.parseUnits('3000', 6));

		await MockERC20.connect(addrs[0]).approve(indexFund.address, ethers.utils.parseUnits('3000', 6));

		let donateTxn = await indexFund
			.connect(donor)
			.depositERC20(donor.address, [1, 50], [0, ethers.utils.parseUnits('3000', 6), registrarConfig.usdcAddress, 'USDC']);
	...
	it('Should add 3000 funding goal to index fund config', async function () {
		...
		let data = indexFund.interface.encodeFunctionData('updateConfig', [[500, 30, ethers.utils.parseUnits('3000', 6)]]);
		...
		expect(config.fundingGoal).to.be.equal(ethers.utils.parseUnits('3000', 6), 'Funding goal should be 3000 USDC');
	});
	...
	it('Should donate to active fund 9000 USDC', async function () {
		...
		await MockERC20.connect(deployer).transfer(addrs[0].address, ethers.utils.parseUnits('9000', 6));

		await MockERC20.connect(addrs[0]).approve(indexFund.address, ethers.utils.parseUnits('9000', 6));

		await indexFund
			.connect(donor)
			.depositERC20(donor.address, [0, 50], [0, ethers.utils.parseUnits('9000', 6), registrarConfig.usdcAddress, 'USDC']);
