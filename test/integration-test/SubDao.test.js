...
describe('SubDao test', function () {
  ...
  it('Should set up sub dao for the endowment', async function () {
    ...
    const data = setUpDao.interface.encodeFunctionData('setupDao', [
      1,
      dao
    ]);
  ...
  it('Should buy subdao token with dai', async function () {
    ...
    let tx1 = await daiToken.connect(deployer).transfer(buyer1.address, ethers.utils.parseUnits('3000', 18));
    let tx2 = await daiToken.connect(deployer).transfer(buyer2.address, ethers.utils.parseUnits('3000', 18));
    let tx3 = await daiToken.connect(deployer).transfer(buyer3.address, ethers.utils.parseUnits('3000', 18));
    let tx4 = await daiToken.connect(deployer).transfer(buyer4.address, ethers.utils.parseUnits('1000', 18));
    ...
    //buyer 1
    await daiToken.connect(buyer1).approve(subdaoToken.address, ethers.utils.parseUnits('3000', 18));
    ...
    //buyer 2
    await daiToken.connect(buyer2).approve(subdaoToken.address, ethers.utils.parseUnits('3000', 18));
    ...
    //buyer 3
    await daiToken.connect(buyer3).approve(subdaoToken.address, ethers.utils.parseUnits('3000', 18));
    ...
    //buyer 4
    await daiToken.connect(buyer4).approve(subdaoToken.address, ethers.utils.parseUnits('1000', 18));
  ...
  it('Should lock subdao token to get 900lv', async function () {
    ...
    console.log('Balance of buyer SD Token before lockup ', (await subdaoToken.balanceOf(buyer1.address)).toString());
    ...
    let tx2 = await lockup.connect(buyer1).createLock(buyer1Balance, parseInt(Date.now() / 1000) + 90 * 24 * 60 * 60);

    await tx2.wait();

    console.log('Balance of buyer SD Token after lockup ', (await subdaoToken.balanceOf(buyer1.address)).toString());

    console.log('Balance of voting Token after lockup', (await lockup.balanceOf(buyer1.address)).toString());

    console.log('Balance of buyer SD Token before lockup ', (await subdaoToken.balanceOf(buyer2.address)).toString());
    ...
    tx2 = await lockup.connect(buyer2).createLock(buyer2Balance, parseInt(Date.now() / 1000) + 90 * 24 * 60 * 60);

    await tx2.wait();

    console.log('Balance of buyer SD Token after lockup ', (await subdaoToken.balanceOf(buyer2.address)).toString());

    console.log('Balance of voting Token after lockup', (await lockup.balanceOf(buyer2.address)).toString());

    console.log('Balance of buyer SD Token before lockup ', (await subdaoToken.balanceOf(buyer3.address)).toString());
    ...
    tx2 = await lockup.connect(buyer3).createLock(buyer3Balance, parseInt(Date.now() / 1000) + 90 * 24 * 60 * 60);

    await tx2.wait();

    console.log('Balance of buyer SD Token after lockup ', (await subdaoToken.balanceOf(buyer3.address)).toString());

    console.log('Balance of voting Token after lockup', (await lockup.balanceOf(buyer3.address)).toString());
  });
  ...
  it('create poll to change the endowment withdraw fees', async function () {
    ...
    const account = await ethers.getContractAt('AccountsUpdateEndowmentSettingsController', deployRes.addresses.account);
    ...
    let data = account.interface.encodeFunctionData('updateEndowmentFees', [UpdateEndowmentFeeRequest]);
    ...
    let tx = await subdao.createPoll(0, 'Creating a poll', 'creating a poll to change the withdrawal fees', 'https://dumylink.com', ExecuteData);
  ...
  it('grant allowance to the subDao to spend funds', async function () {
    ...
    let data = allowance.interface.encodeFunctionData('manageAllowances', [1, 'add', endowment.dao, AllowanceData, registrarConfig.usdcAddress]);

    let tx = await endowmentMultisig
      .connect(admin1)
      .submitTransaction('Set up allowance', 'Set up allowance', deployRes.addresses.account, 0, data);
  ...
  it('spend the fund allowance is granted for', async function () {
    ...
    let AccountsAllowance = await ethers.getContractAt('AccountsAllowance', deployRes.addresses.account);
    ...
    let data = AccountsAllowance.interface.encodeFunctionData('spendAllowance', [1, registrarConfig.usdcAddress, ethers.utils.parseUnits('100', 6), endowment.dao]);
    ...
    let tx = await subdao.createPoll(0, 'Creating a poll', 'creating a poll to withdraw money', 'https://dumylink.com', ExecuteData);
    ...
    assert.equal(parseInt(y.toString()) - parseInt(x.toString()), ethers.utils.parseUnits('100', 6));
  });
  ...
  it('update the config of the endowment', async function () {
    ...
    let data = subdao.interface.encodeFunctionData('updateConfig', [curOwner, curQuorum, curThreshold, curVotingperiod, curTimelockperiod, curExpirationperiod, curProposaldeposit, curSnapshotperiod]);

    let tx = await endowmentMultisig.connect(admin1).submitTransaction('Update config', 'Update config', endowment.dao, 0, data);
  ...
  it('create the poll with the deposit', async function () {
    ...
    let tx1 = await subdao.connect(buyer4).createPoll(ethers.utils.parseUnits('10', 6), 'Creating a poll', 'creating a poll to withdraw money', 'https://dumylink.com', ExecuteData);
  ...
  it('update the registrar contract address', async function () {
    ...
    let data = subdao.interface.encodeFunctionData('registerContract', [curVetoken, curSwapfactory]);
