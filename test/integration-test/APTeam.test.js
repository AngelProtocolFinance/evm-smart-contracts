...
describe("APTeam", function () {
    ...
    this.beforeAll(async function(){ // runs once before all `it` blocks
    ...
    it("should create and pass a proposal in AP multisig to update registrar config", async function(){
        ...
        const data = registrar.interface.encodeFunctionData('updateConfig', [deployRes.registrarConfig]);
        let tx = await APMultisig.connect(admin3).submitTransaction(
            "update registrar config", "update registrar config", registrar.address, 0, data
        );
        let receipt = await tx.wait();

        let confirmations = 0;
        const txId = receipt.events.filter((e) => e.event === "Confirmation")[0].args.transactionId.toString();
        ...
        receipt.events.map((e) => {
            if(e.event === "Confirmation" && e.args.transactionId.toString() === txId)
                confirmations += 1;
            if(e.event === "Execution" && e.args.transactionId.toString() === txId)
                executed = true;
        })
    ...
    it("should create and pass a proposal in AP multisig to update index fund config", async function(){
        ...
        let tx = await APMultisig.connect(admin3).submitTransaction(
            "update index fund config", "update index fund config", indexFund.address, 0, data
        );
        let receipt = await tx.wait();
        let txId = receipt.events.filter((e) => e.event === "Submission")[0].args.transactionId.toString();
