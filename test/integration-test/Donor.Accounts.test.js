...
describe("Donor donating from Accounts", function () {
  ...
  it("Should create a endowment of type normal and deposit usdc by swapping it with ETH", async function () {
    ...
    let depositTxnReceipt = await depositTxn.wait();
    const amountSwapped = depositTxnReceipt.events.filter((e) => e.event === "SwappedToken")[0].args
      .amountOut;
    ...
    expect(lockedBalance.toString(), "Locked balance is 50%").to.equal(
      amountSwapped.div(2).toString()
    );
    expect(liquid_balance.toString(), "Liquid balance is 50%").to.equal(
      amountSwapped.div(2).toString()
    );
  });
