...
describe("Account Update", function () {
  ...
  this.beforeAll(async function () {
    ...
    // deploy eth token
    let usdcAddress = await polygon.deployToken(
      "USDC",
      "USDC",
      6,
      ethers.utils.parseUnits("100000000000000000", 6)
    );
  ...
  it("it should add a network in registrar", async function () {
    ...
    const data1 = registrar.interface.encodeFunctionData("updateNetworkConnections", [
      senderConfig,
      "post",
    ]);
    ...
    receipt1.events.map((e) => {
      if (e.event === "Confirmation" && e.args.transactionId.toString() === txId1)
        confirmations1 += 1;
    });
    ...
    receipt1.events.map((e) => {
      if (e.event === "Confirmation" && e.args.transactionId.toString() === txId1)
        confirmations1 += 1;
      if (e.event === "Execution" && e.args.transactionId.toString() === txId1) executed1 = true;
    });
  ...
  it("should invest into liquid vault", async function () {
    ...
    let account = await ethers.getContractAt("AccountsVaultFacet", deployRes.addresses.account);
    ...
    let queryContract = await ethers.getContractAt(
      "AccountsQueryEndowments",
      deployRes.addresses.account
    );
  ...
  it("should call vault redeem", async function () {
    ...
    let data = account.interface.encodeFunctionData("vaultsRedeem", [1, 1, ["GoldFinch"]]);
  ...
  it("it should receive the callback for Axelar network", async function () {
    ...
    let accountRouter = await ethers.getContractAt(
    ...
    let data = AbiCoder.encode(
      ...
      ["0x6dc4f3f6", "0x6dc4f3f6", [1], registrarConfig.usdcAddress, BigInt(0), BigInt(100e6)]
    );
    ...
    let tx = await accountRouter.executeWithToken(
