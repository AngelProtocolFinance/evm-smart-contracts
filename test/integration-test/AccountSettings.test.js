...
describe('Account Settings Controller', function () {
	...
	it('should update endowment details', async function(){
		...
		let data = account.interface.encodeFunctionData(
			'updateEndowmentDetails',
			[endowmentUpdateData]
		)
		let tx = await endowmentMultisig.connect(admin1).submitTransaction("update endowment details", "update endowment details", account.address, 0, data);
		...
		expect(endowment.image === endowmentUpdateData.image, 'Image updated successfully').to.equal(true);
	});
	it("should update delegate address", async function() {
		...
		let tx = await endowmentMultisig.connect(admin1).submitTransaction("update delegate", "update delegate", account.address, 0, data);
		let txnReceipt = await tx.wait();
	...
	it("should revert with the string `Unauthorized` when trying to update endowment details", async function() {
		...
		await expect(account.connect(addrs[0]).updateEndowmentDetails(endowmentUpdateData)).to.be.revertedWith('Unauthorized');
	});
	...
	it("should update endowment settings through owner", async function() {
		...
		const data = account.interface.encodeFunctionData('updateEndowmentSettings', [endowmentSettingsData]);
		let tx = await endowmentMultisig.connect(admin1).submitTransaction("update endowment settings", "update endowment settings", account.address, 0, data);
		let txnReceipt = await tx.wait();
		let txId = txnReceipt.events[0].args.transactionId;
		// approve the transaction for admin2
		tx = await endowmentMultisig.connect(admin2).confirmTransaction(txId);
	...
	it("should update endowment controller", async function() {
		...
		let tx = await endowmentMultisig.connect(admin1).submitTransaction("updateEndowmentController", "updateEndowmentController", account.address, 0, data);
		let txnReceipt = await tx.wait();
		...
		expect(flag === 1, 'Transaction executed successfully').to.equal(true);
		endowment = await accountQuery.queryEndowmentDetails(1);
		expect(endowment.settingsController.name.govControlled, 'name gov controlled not updated').to.equal(true);
	});
	it("should update endowment fees", async function(){
		...
		let tx = await endowmentMultisig.connect(admin1).submitTransaction("updateEndowmentFees", "updateEndowmentFees", account.address, 0, data);
		let txnReceipt = await tx.wait();
		...
		expect(flag === 1, 'Transaction executed successfully').to.equal(true);
		endowment = await accountQuery.queryEndowmentDetails(1);
		expect(endowment.earningsFee.feePercentage.toString() === "100", 'name gov controlled not updated').to.equal(true);
	});
});
