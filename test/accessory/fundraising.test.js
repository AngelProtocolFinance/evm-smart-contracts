...
describe('Fundraising', function () {
    ...
    it("should update config of fundraising", async function(){
        ...
        taxRate = 20;
        await fundraising.executeUpdateConfig(
        ...
        assert(config.campaignPeriodSeconds.toString() === campaignPeriodSeconds.toString(), "campaignPeriodSeconds should be updated");
        assert(config.taxRate.toString() === taxRate.toString(), "taxRate should be updated");
        assert(config.acceptedTokens.Cw20CoinVerified_addr[0].toString() === projectToken.address, "accpeted_tokens should be updated");
    });
    it("should create a campaign", async function(){
        ...
        campaignId = txReceipt.events.filter((e) => e.event === "FundraisingCampaignCreated")[0].args.campaignId;
        ...
        assert(campaignDetails.description === fundraisingCreateMsg.description, "description should be same");
        assert(campaignDetails.fundingGoal.Cw20CoinVerified_amount[0].toString() === fundraisingCreateMsg.fundingGoal.Cw20CoinVerified_amount[0].toString(), "fundingGoal should be same");
        assert(campaignDetails.fundingThreshold.Cw20CoinVerified_amount[0].toString() === fundraisingCreateMsg.fundingGoal.Cw20CoinVerified_amount[0].mul(fundraisingCreateMsg.rewardThreshold).div(100).toString(), "fundingGoal should be same");
        assert(campaignDetails.lockedBalance.Cw20CoinVerified_amount[0].toString() === balance.cw20Amount.toString(), "lockedBalance should be same");
    });
    it("should execute top up in a campaign", async function(){
        ...
        // approve transfer of project tokens
        await projectToken.approve(fundraising.address, lockedAmount);
        const tx = await fundraising.executeTopUp(
            campaignId,
            balance
        );
        const txReceipt = await tx.wait();
        const campaignDetails = await fundraising.queryDetails(campaignId);
        assert(campaignDetails.lockedBalance.Cw20CoinVerified_amount[0].toString() === balance.cw20Amount.add(lockedAmount).toString(), "lockedBalance should be same");
    ...
    it("should execute contribute in a campaign", async function(){
        ...
        const tx = await fundraising.connect(addrs[0]).executeContribute(
            campaignId,
            balance
        );
        const txReceipt = await tx.wait();
        
        const tx2 = await fundraising.connect(addrs[1]).executeContribute(
            campaignId,
            balance
        );
        const tx2Receipt = await tx2.wait();
        const campaignDetails = await fundraising.queryDetails(campaignId);
        assert(campaignDetails.contributedBalance.Cw20CoinVerified_amount[0].toString() === balance.cw20Amount.mul(2).toString(), "lockedBalance should be same");
    });
    ...
    it("should revert with `Campaign is open` while trying to refund contributions", async function(){
        await expect(fundraising.executeRefundContributions(campaignId)).to.be.revertedWith("Campaign is open");
    });
    it("should close the campaign", async function(){
        ...
        const oldRegistrarTreasuryPTBalance = await projectToken.balanceOf(config.REGISTRAR_DATA.treasury);
        ...
        if(campaignDetails.success){
            const newRegistrarTreasuryPTBalance = await projectToken.balanceOf(config.REGISTRAR_DATA.treasury);
            assert(newPTBalance.sub(oldPTBalance).toString() === 
                contributedBalance.mul(100 - taxRate).div(100).toString(), 
                "PT balance should be updated for deployer");
            assert(newRegistrarTreasuryPTBalance.sub(oldRegistrarTreasuryPTBalance).toString() === 
