...
describe("Staking", function () {
  ...
  it("should stake halo tokens", async function () {
    ...
    assert(
      newStakingTokenBalance.sub(oldStakingTokenBalance).eq(ethers.utils.parseEther("100")),
      "Incorrect staking token balance"
    );
    assert(
      newTotalStaked.sub(oldTotalStaked).eq(ethers.utils.parseEther("100")),
      "Incorrect total staked"
    );
    assert(
      newTotalStakedFor.sub(oldTotalStakedFor).eq(ethers.utils.parseEther("100")),
      "Incorrect total staked for"
    );
  });
  it("should unstake halo tokens", async function () {
    ...
    assert(
      newHaloTokenBalance.sub(oldHaloTokenBalance).eq(
        ethers.utils
          .parseEther("100")
          .mul(100 + interestRate)
          .div(100)
      ),
      "Incorrect halo token balance"
    );
    assert(
      oldStakingTokenBalance.sub(newStakingTokenBalance).eq(ethers.utils.parseEther("100")),
      "Incorrect staking token balance"
    );
    assert(
      oldTotalStaked.sub(newTotalStaked).eq(ethers.utils.parseEther("100")),
      "Incorrect total staked"
    );
    assert(
      oldTotalStakedFor.sub(newTotalStakedFor).eq(ethers.utils.parseEther("100")),
      "Incorrect total staked for"
    );
    MockDate.reset();
  });
