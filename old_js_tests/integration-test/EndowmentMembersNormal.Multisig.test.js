...
describe("Endowment Members", function () {
  ...
  it("Should replace admin3 with new owner", async function () {
    ...
    const data = endowmentMultisig.interface.encodeFunctionData("replaceOwner", [
      admin3.address,
      admin4.address,
    ]);
    ...
    expect(owners, "Admin3 replaced with admin4").to.deep.equal([
      admin1.address,
      admin2.address,
      admin4.address,
    ]);
  ...
  it("add a new owner admin 3 in the endowment multisig", async function () {
    ...
    expect(owners, "Admin3 added to owners").to.deep.equal([
      admin1.address,
      admin2.address,
      admin3.address,
    ]);
    ...
    expect(confirmations, "Admin1 and admin2 confirmed the transaction").to.deep.equal([
      admin1.address,
      admin2.address,
    ]);
  });
