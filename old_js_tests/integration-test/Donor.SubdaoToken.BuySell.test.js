...
describe("Donor buy sell subdao token", function () {
  ...
  it("Should setup endowment with bancor bonding curve subdao token linear", async function () {
    ...
    let tx3 = await daiToken
      .connect(deployer)
      .transfer(buyer.address, ethers.utils.parseUnits("6000", 18));
  ...
  it("Should buy subdao token, lock it in incentivsed voting lockup and query voting balance", async function () {
    ...
    let tx2 = await lockup
      .connect(buyer)
      .createLock(subdaoTokenBalance, parseInt(Date.now() / 1000) + 90 * 24 * 60 * 60);

    await tx2.wait();

    console.log(
      "Balance of buyer SD Token after lockup ",
      (await subdaoToken.balanceOf(buyer.address)).toString()
    );

    console.log(
      "Balance of voting Token after lockup",
      (await lockup.balanceOf(buyer.address)).toString()
    );
  ...
  it("should increase lock amount", async function () {
    ...
    let tx3 = await daiToken
      .connect(deployer)
      .transfer(buyer.address, ethers.utils.parseUnits("600", 18));
  ...
  it("should increase lock time", async function () {
    ...
    let tx = await lockup
      .connect(buyer)
      .increaseLockLength(parseInt(Date.now() / 1000) + 300 * 24 * 60 * 60);
