...
describe("Collector", function () {
  ...
  it("should sweep the collected tokens", async function () {
    ...
    let total = govContractBalance
      .add(distContractBalance)
      .sub(oldGovContractBalance)
      .sub(oldDistContractBalance);

    assert(
      total.mul(rewardFactor).div(100).div(100).toString() ===
        govContractBalance.sub(oldGovContractBalance).div(100).toString(),
      "Incorrect gov contract balance"
    ); // extra div by 100 to round off the decimals

    assert(
      total
        .mul(100 - rewardFactor)
        .div(100)
        .div(100)
        .toString() === distContractBalance.sub(oldDistContractBalance).div(100).toString(),
      "Incorrect dist contract balance"
    ); // extra div by 100 to round off the decimals
  });
  it("should update config of the collector contract", async function () {
    ...
    const config = await collector.queryConfig();
    assert(
      config.rewardFactor.toString() === "80" &&
        config.timelockContract === tempGov.address &&
        config.govContract === addrs[4].address &&
        config.swapFactory === swapping.address,
      "Incorrect config set"
    );
  });
