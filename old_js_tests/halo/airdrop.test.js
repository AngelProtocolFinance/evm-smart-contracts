...
describe("Airdrop", function () {
  ...
  it("should initialize the airdrop contract by calling the initialize function", async function () {
    ...
    assert(
      config.owner === owner.address && config.haloToken == haloToken.address,
      "Incorrect config set"
    );
  });
  it("should not allow non-owner to update config", async function () {
    await expect(airdrop.connect(addrs[0]).updateConfig(owner.address)).to.be.revertedWith(
  ...
  it("should claim tokens", async function () {
    for (let i = 0; i < 3; i++) {
      ...
      assert(
        newHaloTokenBalance.sub(oldHaloTokenBalance).toString() === amounts[i].toString(),
